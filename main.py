...
MAIN_SUB_HEADER = f"{H1}Main Settings{NEW_LINE}"
...
def write_main_settings(main_config):
    ...
    for menu_items, section_config in main_config.items():
        save_file(MAIN_SAVE_LOCATION, f"{H2}{menu_items}{NEW_LINE}", "a")
        ...
        for sub_menu, sub_menu_values in individual_menu:
            ...
            save_file(MAIN_SAVE_LOCATION, f"{H3}{sub_menu}{NEW_LINE}", "a")

            ini_config = sub_menu_values.items()
            for key, value in ini_config:
                save_file(MAIN_SAVE_LOCATION, f"{H4}{key}{NEW_LINE}", "a")
                if hasattr(value, "val"):
                    save_file(MAIN_SAVE_LOCATION, f"{value.comment[2:]}{NEW_LINE}", "a")
...
def write_shader_settings(shader_config):
    for menu_items, section_config in shader_config.items():
        ...
        save_file(f"{SHADERS_SAVE_LOCATION}{menu_items}", f"{H1}{menu_items}{NEW_LINE}", "a")
        save_file(f"{SHADERS_SAVE_LOCATION}{menu_items}", UNDERLINE, "a")

        individual_menu = section_config.items()
        for sub_menu, sub_menu_values in individual_menu:
            save_file(f"{SHADERS_SAVE_LOCATION}{menu_items}", f"{H2}{sub_menu}{NEW_LINE}", "a")

            ini_config = sub_menu_values.items()
            for key, value in ini_config:
                save_file(f"{SHADERS_SAVE_LOCATION}{menu_items}", f"{H3}{key}{NEW_LINE}", "a")
                if hasattr(value, "val"):
                    save_file(f"{SHADERS_SAVE_LOCATION}{menu_items}", f"{value.comment[2:]}{NEW_LINE}", "a")
